...
def insert_json_data(json_data, connection_params):
    ...
    insert_query = """
    INSERT INTO teams (
        team_id, team_name, team_gender, team_group, country_name
    ) VALUES (
        %s, %s, %s, %s, %s
    )
            ...
            for item in json_data:
                for team_key in ["home_team", "away_team"]:
                    ...
                    cur.execute(insert_query, (
                        team.get("home_team_id") if team_key == "home_team" else team.get("away_team_id"),
                        team.get("home_team_name") if team_key == "home_team" else team.get("away_team_name"),
                        team.get("home_team_gender") if team_key == "home_team" else team.get("away_team_gender"),
                        team.get("home_team_group") if team_key == "home_team" else team.get("away_team_group"),
                        team.get("country", {}).get("name"),
                    ))
